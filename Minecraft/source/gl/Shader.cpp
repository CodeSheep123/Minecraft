...
Shader::Shader() : ID(static_cast<unsigned>(-1)), loaded(false) {}
...
Shader::Shader(Shader&& other)
{
	...
	other.ID = static_cast<unsigned>(-1);
	other.loaded = false;
}
...
void Shader::load(std::string const& vertex_path, std::string const& fragment_path)
{
	...
	{
		std::cout << "SHADER::LOAD::FATAL_ERROR: Failure while reading shader source files: " << e.what() << "\n";
		std::cout << "File: " << vertex_path << " " << fragment_path << "\n";
	...
	if (!success)
	{
		...
		std::cout << "In file: " << vertex_path << "\n";
		std::cout << "SHADER::VERTEX_SHADER::COMPILATION_FAILED: " << error << "\n";
	...
	if (!success)
	{
		...
		std::cout << "In file: " << fragment_path << "\n";
		std::cout << "SHADER::FRAGMENT_SHADER::COMPILATION_FAILED: " << error << "\n";
		throw Exception(error);
	};
	...
	if (!success)
	{
		...
		std::cout << "File: " << vertex_path << " " << fragment_path << "\n";
		std::cout << "SHADER::LINK_FAILED: " << error << "\n";
		throw Exception(error);
	}
